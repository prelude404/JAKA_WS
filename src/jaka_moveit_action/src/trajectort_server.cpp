...
void jakacontroller_execute(const jaka_moveit_action::jakacontrollerGoalConstPtr& goal, Server *ac)
{
    ...
    int total_point_num = goal->point_count;//转角序列总步数
    ...
    std::vector<float> Joint_Series;//关节转角序列
    ...
    {
        //std::cout << "Robot_Connect_Flag && !Robot_Move_Flag" << std::endl;
        ...
        for(point_num = 0; point_num < total_point_num; point_num++)
        {
            ...
            robot.servo_j(&joint_goal, ABS);//关节空间位置控制（一步）
            usleep(16 * 1000);
